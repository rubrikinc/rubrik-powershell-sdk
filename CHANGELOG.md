...
- Fix `Get-RscHost` and `Get-RscFilesetTemplate`: resolve GraphQL 400 validation errors caused by
  ...
  query string construction with `RscGqlOperation` to keep signatures in sync with the schema.
...
- `Get-RscHelp` default search now shows queries and mutations with
  their return types (`Kind | GqlField | ReturnType`) for quick GraphQL
  field discovery (#227)
- `Get-RscHelp -Stats` shows schema statistics: element counts, top
  return types, largest interfaces, and type coverage
