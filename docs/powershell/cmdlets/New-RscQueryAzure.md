...
- There are 7 arguments.
    ...
    - managementGroupCustomerIds - list of System.Strings: Filter subscriptions by management group customer IDs. When provided, only subscriptions belonging to these management groups are returned. Empty or not set means no filter (all subscriptions included).
...
for RCV Azure BLI migration details.
    ...
    - sortOrder - SortOrder: Sort order (ASC/DESC) for BLI migration details.
