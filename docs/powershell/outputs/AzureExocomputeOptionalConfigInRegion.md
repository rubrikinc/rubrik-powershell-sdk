...
- azureSqlPrivateDnsZoneId: System.String
  - Azure resource ID of the private DNS zone which will be used to resolve
private endpoints from the exocompute subnet to Rubrik-managed Azure SQL
Servers.
NOTE:
This is different from:
- `private_dns_zone_id` which is for blob storage private
endpoints (`privatelink.blob.core.windows.net`).
- `aks_custom_private_dns_zone_id` which is for AKS API server
resolution (`privatelink.<region>.azmk8s.io`).
The name of this DNS zone must be exactly
`privatelink.database.windows.net`.
- diskEncryptionSetId: System.String
  - Azure resource ID of the Disk Encryption Set to be used for encrypting
the OS and data disks of the AKS cluster nodes.
