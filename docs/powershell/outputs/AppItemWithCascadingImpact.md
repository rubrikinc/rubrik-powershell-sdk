...
- appItemTypeToken: System.String
  - Token specifying the type of the item. The token should exactly match the token retrieved from the query GraphQL field response.
...
- relationshipType: RelationshipType
  - The type of the relationship between this app item type and the one immediately above it in the cascading hierarchy.
...
- itemsWithActionType: list of cascadingImpactKeyss
  - Information about the keys for this app item type, including the action to perform on each key during a restore.
