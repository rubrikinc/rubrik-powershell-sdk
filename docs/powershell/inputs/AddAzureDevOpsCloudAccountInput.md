...
Vault). BYOS requires backup_location_id; RCV auto-provisions storage.
- exocomputeRegion: System.String
  - Azure region for Rubrik-hosted exocompute (e.g., "eastus", "westus2").
Required when host_type is RUBRIK_HOST. Must be in the same region as
the archival location when storage_type is BYOS.
