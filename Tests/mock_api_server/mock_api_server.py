...
def normalize_query(query_string: str) -> str:
    LOG.debug("Before: %s", query_string)
    query_string = query_string.replace("\t", "")
    query_string = query_string.replace("\n", "")
    query_string = query_string.replace(" ", "")
    query_string = query_string.replace("__typename", "")
    LOG.debug("After: %s", query_string)
    return query_string


def get_key(body):
    LOG.debug("Getting cache key for request body: %s", body)
    variables = ""
    query = ""
    ...
    if "query" in body:
        LOG.debug("query: %s", body["query"])
        query = body["query"]
        query = normalize_query(query[query.index("{") + 1 : query.rindex("}")])
    if "variables" in body:
        LOG.debug("variables: %s", body["variables"])
...
class Cache:
    ...
    def load_from_file(self, filename):
        with open(filename, "r", encoding="utf-8") as f:
            self.update(json.load(f), normalize_keys=False)
    ...
    def update(self, new_cache: dict, normalize_keys=True):
        for request in new_cache:
            if request not in self.cache:
                self.cache[request] = {}
            for body in new_cache[request]:
                response = new_cache[request][body]
                key = get_key(body) if normalize_keys else body
                if key not in self.cache[request]:
                    LOG.debug("Adding new key: %s", key)
                    self.cache[request][key] = {}
                else:
                    LOG.debug("Updating existing key: %s", key)
    ...
    def get(self, request, payload: dict = None):
        ...
        if request not in self.cache:
            ...
            LOG.debug("Found request: %s", request)

        key = get_key(payload)
        if key not in self.cache[request]:
            LOG.info("%s: cache key not found: %s", request, key)
            return {
...
class CacheHandler(BaseHTTPRequestHandler):
    ...
    def handle_request(self, method):
        ...
        if request_key == "POST /cache":
            ...
            shutdown_api_server()
            self._send_response({})
        else:  # in particular, POST /api/graphql is handled here
