...
Describe -Name "Test variables" -Fixture {
    It -Name "With New-RscQueryMssql -Op RecoverableRanges" -Test {
        # Make sure the schema hasn't changed:
        $varInfo = (New-RscQueryMssql -Op RecoverableRanges).Var.Info()
        ...
        # No variables
        $vars = @{}
        $serialized = (New-RscQueryMssql -Op RecoverableRanges -Var $vars).GqlRequest().Variables
        ...
        $vars = (New-RscQueryCluster -Op List -Var @{first = 1 }).Var.ToString()
        ...
        $vars = (New-RscQueryCluster -Op List -Var @{first = 1; after = "2" }).Var.ToString()
        $q = New-RscQueryCluster -Op List -Var "first=1,after=2"
        $q.Var.ToString() | Should -Be $vars
        $q = New-RscQueryCluster -Op List -Var @{First = 1; After = "2" }
        $q.Var.ToString() | Should -Be $vars 
        $q = New-RscQueryCluster -Op List -Var "First=1,After=2"
        ...
        # Enum values
        $q1 = New-RscQueryCluster -Op Connected -Var @{"clusterFilterArg" = [RubrikSecurityCloud.Types.ClusterTypeEnum]::POLARIS }
        $q2 = New-RscQueryCluster -Op Connected -Var ([RubrikSecurityCloud.Types.ClusterTypeEnum]::POLARIS)
