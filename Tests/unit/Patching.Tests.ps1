...
Describe -Name "Test patching" -Fixture {
    It -Name "With New-RscQueryCluster -List" -Test {
        ...
        $cdmUpgradeInfoFieldSpec = ((New-RscQueryCluster -List -AddField Nodes.CdmUpgradeInfo).Field.Nodes.CdmUpgradeInfo.AsFieldSpec()).Replace("`n", " ").Trim()
        $cdmUpgradeInfo = "cdmUpgradeInfo { " + $cdmUpgradeInfoFieldSpec + " }"

        # Start with the default
        $q0 = New-RscQueryCluster -List
        $q0Fs = FieldSpec($q0)
        $q0ExpectedFs = "count nodes { $defaultNodes } $pageInfo"
        $q0Fs | Should -Be $q0ExpectedFs

        # Add a scalar
        $q1 = New-RscQueryCluster -List -AddField nodes.snapshotcount
        $q1Fs = FieldSpec($q1)
        ...
        # Take it back
        $qc3 = New-RscQueryCluster -List -Copy $qc2 -RemoveField nodes.estimatedrunway
        $qc3Fs = FieldSpec($qc3)
        $qc3ExpectedFs = $qc1ExpectedFs
        $qc3Fs | Should -Be $qc3ExpectedFs

        # Add an object, via copy
        $qc4 = New-RscQueryCluster -List -Copy $qc3 -AddField nodes.cdmupgradeinfo
        $qc4Fs = FieldSpec($qc4)
        ...
        FieldSpec(New-RscQueryCluster -List -Copy $qc2 -RemoveField nodes.estimatedrunway) | Should -Be $qc3ExpectedFs

        # Take it back
        $qc5 = New-RscQueryCluster -List -Copy $qc4 -RemoveField nodes.cdmupgradeinfo
        $qc5Fs = FieldSpec($qc5)
        $qc5ExpectedFs = "count nodes { $defaultNodesSnapshot } $pageInfo"
        $qc5Fs | Should -Be $qc5ExpectedFs

        # Start with a field object
        $qf1FieldName = (New-RscQueryCluster -List).Field.GetType().Name
        $qf1Field = Get-RscType -Name $qf1FieldName
        $qf1Field.SelectForRetrieval()
        $qf1 = New-RscQueryCluster -List -Field $qf1Field
        $qf1Fs = FieldSpec($qf1)
        $qf1ExpectedFs = "count nodes { $defaultNodes } $pageInfo"
        $qf1Fs | Should -Be $qf1ExpectedFs
        
        # Add a scalar to a field object
        $qf2 = New-RscQueryCluster -List -Field $qf1Field -AddField nodes.snapshotcount
        $qf2Fs = FieldSpec($qf2)
        ...
        FieldSpec(New-RscQueryCluster -List -Field $qf1Field) | Should -Be $qf1ExpectedFs

        # Add another scalar to a field object
        $qf2b = New-RscQueryCluster -List -Field $qf2.Field -AddField nodes.estimatedrunway
        $qf2bFs = FieldSpec($qf2b)
        $qf2bExpectedFs = $qc2ExpectedFs
        # But it doesn't modify the original field object
        FieldSpec(New-RscQueryCluster -List -Field $qf2.Field) | Should -Be $qf2ExpectedFs

        # Take first one back
        $qf3 = New-RscQueryCluster -List -Field $qf2b.Field -RemoveField nodes.snapshotcount
        $qf3Fs = FieldSpec($qf3)
        ...
        $qf4 = New-RscQueryCluster -List -Field $qf3.FIeld -AddField nodes.cdmupgradeinfo
        $qf4Fs = FieldSpec($qf4)
        $qf4ExpectedFs = "count nodes { $defaultNodesEstimatedRunway $cdmUpgradeInfo } $pageInfo"
        $qf4Fs | Should -Be $qf4ExpectedFs

        # Take it back
        $qf5 = New-RscQueryCluster -List -Field $qf4.Field -RemoveField nodes.cdmupgradeinfo
        $qf5Fs = FieldSpec($qf5)
        ...
        $qe1 = New-RscQueryCluster -List -FieldProfile EMPTY -AddField Count
        $qe2 = New-RscQueryCluster -List -RemoveField Nodes, PageInfo
