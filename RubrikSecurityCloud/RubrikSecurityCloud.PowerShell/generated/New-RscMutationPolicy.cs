...
namespace RubrikSecurityCloud.PowerShell.Cmdlets
{
    ...
    ///
    /// <example>
    /// Runs the SetAirPolicyAlertStatus operation
    /// of the 'Policy' API domain.
    /// <code>
    /// PS &gt;
    ///
    ...
    /// # Create an RscQuery object for:
    /// # API Domain:    Policy
    /// # API Operation: SetAirPolicyAlertStatus
    /// 
    /// $query = New-RscMutationPolicy -Operation SetAirPolicyAlertStatus
    /// 
    /// # REQUIRED
    /// $query.Var.input = @{
    /// 	# REQUIRED
    ...
    /// 	)
    /// 	# REQUIRED
    /// 	status = $someAirPolicyViolationStatus # Call [Enum]::GetValues([RubrikSecurityCloud.Types.AirPolicyViolationStatus]) for enum values.
    /// }
    /// 
    /// # Execute the query
    /// 
    /// $result = $query | Invoke-Rsc
    /// 
    /// Write-Host $result.GetType().Name # prints: SetAirPolicyAlertStatusReply
    /// 
    /// 
    /// 
    /// </code>
    ...
    public class New_RscMutationPolicy : RscGqlPSCmdlet
    {
            ...
            try
            {
                switch(this.GetOp().OpName())
                {
                    ...
                    case "SeedInitialPolicies":
                        ...
                        break;
                    case "SetAirPolicyAlertStatus":
                        this.ProcessRecord_SetAirPolicyAlertStatus();
                        break;
        ...
        // This parameter set invokes a single graphql operation:
        // setAirPolicyAlertStatus.
        internal void ProcessRecord_SetAirPolicyAlertStatus()
        {
            this._logger.name += " -SetAirPolicyAlertStatus";
            // Create new graphql operation setAirPolicyAlertStatus
            InitMutationSetAirPolicyAlertStatus();
        }
...
$query.Var.runAsync = $someBoolean"
        ...
        // Create new GraphQL Mutation:
        // setAirPolicyAlertStatus(input: SetAirPolicyAlertStatusInput!): SetAirPolicyAlertStatusReply!
        internal void InitMutationSetAirPolicyAlertStatus()
        {
            Tuple<string, string>[] argDefs = {
                Tuple.Create("input", "SetAirPolicyAlertStatusInput!"),
            };
            Initialize(
                argDefs,
                "mutation",
                "MutationSetAirPolicyAlertStatus",
                "($input: SetAirPolicyAlertStatusInput!)",
                "SetAirPolicyAlertStatusReply",
                Mutation.SetAirPolicyAlertStatus,
                Mutation.SetAirPolicyAlertStatusFieldSpec,
                @"# REQUIRED
$query.Var.input = @{
	# REQUIRED
	alertIds = @(
