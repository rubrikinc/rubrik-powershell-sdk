...
namespace RubrikSecurityCloud.Types
{
    ...
    public class GetExotaskImageBundleReply: BaseType
    {
        ...
        //      C# -> AwsExoTaskImageBundle? AwsImages
        // GraphQL -> awsImages: AWSExoTaskImageBundle (type)
        [JsonProperty("awsImages")]
        public AwsExoTaskImageBundle? AwsImages { get; set; }

        //      C# -> AzureExoTaskImageBundle? AzureImages
        // GraphQL -> azureImages: AzureExoTaskImageBundle (type)
        [JsonProperty("azureImages")]
        public AzureExoTaskImageBundle? AzureImages { get; set; }
    ...
    ) 
    {
        ...
        if ( AwsImages != null ) {
            this.AwsImages = AwsImages;
        }
        if ( AzureImages != null ) {
            this.AzureImages = AzureImages;
        }
    ...
    public override string AsFieldSpec(FieldSpecConfig? conf=null)
    {
        ...
        if (this.AwsImages != null) {
            var fspec = this.AwsImages.AsFieldSpec(conf.Child("awsImages"));
        ...
        if (this.AzureImages != null) {
            var fspec = this.AzureImages.AsFieldSpec(conf.Child("azureImages"));
            if(fspec.Replace(" ", "").Replace("\n", "").Length > 0) {
                if (conf.Flat) {
                    s += conf.Prefix + fspec;
                } else {
    ...
    public override void ApplyExploratoryFieldSpec(AutofieldContext ec)
    {
        ...
        //      C# -> AwsExoTaskImageBundle? AwsImages
        // GraphQL -> awsImages: AWSExoTaskImageBundle (type)
        if (ec.Includes("awsImages",false))
        {
            if(this.AwsImages == null) {

                this.AwsImages = new AwsExoTaskImageBundle();
                this.AwsImages.ApplyExploratoryFieldSpec(ec.NewChild("awsImages"));
        ...
        else if (this.AwsImages != null && ec.Excludes("awsImages",false))
        {
            this.AwsImages = null;
        }
        //      C# -> AzureExoTaskImageBundle? AzureImages
        // GraphQL -> azureImages: AzureExoTaskImageBundle (type)
        if (ec.Includes("azureImages",false))
        {
            if(this.AzureImages == null) {

                this.AzureImages = new AzureExoTaskImageBundle();
                this.AzureImages.ApplyExploratoryFieldSpec(ec.NewChild("azureImages"));
        ...
        else if (this.AzureImages != null && ec.Excludes("azureImages",false))
        {
            this.AzureImages = null;
        }
        //      C# -> List<BundleImage>? BundleImages
        // GraphQL -> bundleImages: [BundleImage!]! (type)
