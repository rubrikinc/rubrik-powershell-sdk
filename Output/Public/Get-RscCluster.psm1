...
function Get-RscCluster {
    ...
    Process {
        ...
        $inputs = (Invoke-RscQueryCluster -Op $operation -InputProfile $inputProfile -GetInputs)

        # -4- Customize input objects:
        if ( $Id ) {
            $inputs.Arg.clusterUuid = $Id
        }
        else {
            if ( $First -gt 0 ) {
        ...
        $result = Invoke-RscQueryCluster -Op $operation -Field $inputs.Field -Arg $inputs.Arg
